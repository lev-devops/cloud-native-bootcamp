...
- Mac: existing source workspace plus `.local/environments/<profile>/bootstrap.json` (ignored local state).
...
The wizard supports interactive and scripted operation. For automation, provide `--non-interactive --name --host --remote-dir --yes`; `--yes` is required with `--check` or `--apply`. This is workspace bootstrap, not the complete stack installer. It does not copy source, install packages, initialize Git, configure mise in a new project, run sudo, start services, or deploy Kubernetes workloads. The current project remains at its existing path; it is not renamed.
